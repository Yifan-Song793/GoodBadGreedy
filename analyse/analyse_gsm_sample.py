...
def main(args):
    ...
    for model in models:
        ...
        for sample_idx in tqdm(range(sample_num)):
            if not os.path.exists(os.path.join(args.data_path, f"{model}-cot-8shot-sample-{sample_idx}", "predictions.jsonl")):
                continue
            if not os.path.exists(os.path.join(args.data_path, f"{model}-cot-8shot-sample-{sample_idx}", "metrics.json")):
                continue
            with open(os.path.join(args.data_path, f"{model}-cot-8shot-sample-{sample_idx}", "predictions.jsonl")) as fr:
            ...
            for idx, item in enumerate(cur_raw_annotation):
                ...
                try:
                    cur_len.append(len(encoder.encode(item['model_output'])))
                except:
