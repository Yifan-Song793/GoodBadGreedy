...
def main(args):
    ...
    for sample_idx in range(args.sample_num):
        with open(os.path.join(args.completion_path, f"{model}-cot-8shot-sample-{sample_idx}", "predictions.jsonl")) as fr:
            cur_raw_annotation = [json.loads(line) for line in fr]
        test_samples = []
        for idx, item in enumerate(cur_raw_annotation):
            prompt = item['question']
            completion = item['model_output']
            cur_msg = [
                {
                    "role": "user",
                    "content": prompt,
        ...
        for idx, (item, reward) in enumerate(zip(cur_raw_annotation, rewards)):
            idx = str(idx)
    ...
    if not os.path.exists(os.path.join(args.output_path, model, "gsm")):
        os.mkdir(os.path.join(args.output_path, model, "gsm"))

    output_path = os.path.join(args.output_path, model, "gsm", f"{reward_model_name}.json")
